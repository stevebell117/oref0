...
function verify_value(item, verify_not_null = false) {
...
function get_sensitivity_ratio(
    profile
) {
...
function get_sens(
    ...
    var sensitityRatio = profile.sens;
    if (verify_value(autosens_data, true)) {
...
function get_average_predicted_bg(
    meal_data,
    minUAMPredBG,
    minCOBPredBG,
    fractionCarbsLeft,
...
var determine_basal = function determine_basal(
    ...
    var deliverAt = (currentTime) ? currentTime : new Date();
    var systemTime = (currentTime) ? currentTime : new Date();

    if (!verify_value(profile) || !verify_value(profile.current_basal)) {
    ...
    */
    ...
    rT = check_for_errored_bg(
        bg,
        glucose_status,
        currenttemp
    )

    // We need to return if the requestedTemp has been set for any reason.
    if (verify_value(rT.reason)) {
        return rT;
    }
    ...
    var target_bg = (verify_value(profile.min_bg) && verify_value(profile.max_bg)) ? (profile.min_bg + profile.max_bg) / 2 : null;
    if (!target_bg) {
    ...
    var min_bg = verify_value(profile.min_bg) ? profile.min_bg : null;
    var max_bg = verify_value(profile.max_bg) ? profile.max_bg : null;
    
    // Calculate sensitivityRatio based on temp targets, if applicable, or using the value calculated by autosens
    var sensitivityRatio = get_sensitivity_ratio(profile);

    // Conversely, adjust BG target based on autosens ratio if no temp target is running
    // adjust min, max, and target BG for sensitivity, such that 50% increase in ISF raises target from 100 to 120
    if (!profile.temptargetSet && verify_value(autosens_data, true)) {
    ...
    if (verify_value(iob_data)) {
        rT.error ='Error: iob_data undefined. ';
        return rT;
    }

    var iobArray = iob_data;
    if (verify_value(iob_data.length) && iob_data.length > 1) {
        iob_data = iobArray[0];
        //console.error(JSON.stringify(iob_data[0]));
    }

    if (!verify_value(iob_data.activity) || !verify_value(iob_data.iob)) {
        rT.error ='Error: iob_data missing some property. ';
        return rT;
    }

    // Prep various delta variables. TODO: make this happen earlier along with other variable prep
    var tick = (glucose_status.delta > -0.5) ? "+" + round(glucose_status.delta,0) : round(glucose_status.delta,0);
    ...
    var sens = get_sens(profile); //TODO: stebell - this is broken.
    ...
    var lastTempAge = 0;
    if (verify_value(iob_data.lastTemp)) {
    ...
    if (verify_value(eventualBG) || isNaN(eventualBG)) {
    ...
    avgPredBG = get_average_predicted_bg(
        meal_data,
        minUAMPredBG,
        minCOBPredBG,
        fractionCarbsLeft,
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        if (verify_value(currenttemp.rate) && (currenttemp.duration > 5 && rate >= currenttemp.rate * 0.8)) {
